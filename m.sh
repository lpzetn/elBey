...
# cat sites.txt | xargs -n 1 -P 42 bash -c 'fetch_website "$0"'
...
echo "website,email" > output.csv

fetch_website() {
    website=$1
    # Log progress to stderr so it doesn't pollute the CSV stdout pipe
    ...
    if [[ -n "$email_found" ]]; then
        emails_joined=$(echo "$email_found" | paste -sd "; " -)
        echo "\"$website\",\"$emails_joined\""
        return
    fi
...
# Stream xargs results cleanly into output.csv
cat sites.txt | xargs -n 1 -P 42 bash -c 'fetch_website "$0"' >> output.csv
